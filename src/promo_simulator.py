...
def find_optimal_promos(sim_results, optimize_for="promo_roi"):
    ...
    """
    # Filter to positive ROI scenarios
    positive = sim_results[sim_results["promo_roi"] > 0].copy()

    if positive.empty:
        print("Warning: No positive ROI scenarios found. Using all results.")
        positive = sim_results.copy()

    optimal = positive.loc[
        positive.groupby(["category", "segment"])[optimize_for].idxmax()
...
def generate_recommendations(optimal_promos, sim_results):
    ...
    top_roi = meaningful.nlargest(3, "promo_roi")
